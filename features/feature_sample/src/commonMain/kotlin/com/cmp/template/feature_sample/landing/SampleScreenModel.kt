...
class SampleScreenModel(
...
) : BaseScreenModel<SampleScreenState>(SampleScreenState()), SampleScreenEvent {

    init { onLoadItems() }

    override fun onLoadItems() {
        updateState { copy(isLoading = true, error = null) }
        viewModelScope.launch {
            getSampleItemsUseCase.execute(Unit) { result ->
                when (result) {
                    is StatefulResult.Success -> updateState { copy(items = result.data, isLoading = false) }
                    is StatefulResult.Error   -> updateState { copy(error = result.error.message, isLoading = false) }
    ...
    override fun onRefresh() = onLoadItems()
}
